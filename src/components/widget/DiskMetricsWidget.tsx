...
export interface DiskMetricsWidgetProps extends Omit<WidgetProps, 'children'> {
  nodeId?: string;
  minWidth?: string;
  minHeight?: string;
}
...
const DiskMetricsWidget: React.FC<DiskMetricsWidgetProps> = ({
  nodeId,
  minWidth = '300px',
  minHeight = '200px',
  ...widgetProps
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      socket.onopen = () => {
        console.log(`📡 WebSocket 연결됨 - 디스크 메트릭 위젯 (노드: ${selectedNodeId})`);
        setConnected(true);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          const diskMetrics = {
            ...
            metrics: { disk_usage: data.metrics.disk_usage },
            last_update: data.last_update
          };
          
          setMetrics(diskMetrics);
      ...
      socket.onclose = (event) => {
        console.log(`🔌 WebSocket 연결 종료 - 디스크 메트릭 위젯 (노드: ${selectedNodeId})`);
        setConnected(false);
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    ...
                    >
                      ...
                      <Tooltip 
                        ...
                        formatter={(value: number) => [`${value.toFixed(2)}%`, '디스크 사용률']}
                      />
...
export default DiskMetricsWidget; 