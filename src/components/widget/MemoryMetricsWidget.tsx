...
export interface MemoryMetricsWidgetProps extends Omit<WidgetProps, 'children'> {
  nodeId?: string;
  minWidth?: string;
  minHeight?: string;
}
...
const MemoryMetricsWidget: React.FC<MemoryMetricsWidgetProps> = ({
  nodeId,
  minWidth = '300px',
  minHeight = '200px',
  ...widgetProps
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      socket.onopen = () => {
        console.log(`📡 WebSocket 연결됨 - 메모리 메트릭 위젯 (노드: ${selectedNodeId})`);
        setConnected(true);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          const memoryMetrics = {
            ...
            metrics: { memory_usage: data.metrics.memory_usage },
            last_update: data.last_update
          };
          
          setMetrics(memoryMetrics);
      ...
      socket.onclose = (event) => {
        console.log(`🔌 WebSocket 연결 종료 - 메모리 메트릭 위젯 (노드: ${selectedNodeId})`);
        setConnected(false);
...
export default MemoryMetricsWidget; 